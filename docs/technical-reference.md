# Referencia técnica de agentic-core

`@kroxidev/agentic-core` instala una capa autónoma para Codex y una unidad Python 3.11+ (esquema 3), con runtime y herramientas privados. Directo aplica las Golden Rules y las comprobaciones pertinentes del encargo. La verificación de Light y Normal exige tests funcionales y solo controles explícitos y puede emitir `QUALITY_OK`; Full está deprecado; su contenido se conserva en [archive/full](full-archive.md). Las secciones de coordinación y `QualitySession` del esquema 2 se conservan para instalaciones anteriores.
...
La salida muestra el alcance y los archivos de tests ejecutados. `AGENTIC_CORE_OUTPUT=json` añade `selection`, inventario, identidad de ejecución, cobertura y `suite.executed` (ruta, identificador opaco y resultado por test, sin publicar parámetros). Cambiar código o tests seleccionados cambia la identidad de evidencia; esta ejecución no publica un recibo `QUALITY_OK` ni reemplaza aprobaciones de tareas. Los fallos funcionales se rechazan. La falta de ejecución real, los límites y los cambios de inputs impiden aprobar. La cobertura es informativa para los tests funcionales; C.R.A.P. y mutación sí exigen su evidencia de cobertura. `test --changes` selecciona los cambios respecto del inicio conservado de la tarea.
...
`verify --control dry --scope src` compara el inicio conservado con el código actual, incluso si se solicita después de los cambios; también admite archivos o `--changes`. La duplicación previa sin cambios se informa; los candidatos nuevos o modificados sin resolución bloquean. Sin una referencia válida y medible no se aprueba el diferencial. El alcance debe estar dentro del código capturado al preparar. `dry --scope src` sigue siendo un análisis autónomo del estado actual y no aprueba implementaciones. Los tests funcionales deben pasar, pero su falta de cobertura no bloquea DRY ni el cierre sin controles; la cobertura disponible se informa por separado. C.R.A.P. y mutación mantienen sus requisitos de cobertura.

`verify --control crap --scope src --test tests/test_subject.py` mide C.R.A.P. actual y el inicio conservado, ejecutando los tests seleccionados también en una copia de los inputs iniciales. Admite petición tardía y `--changes`. Informa deuda previa sin exigir repararla; el código nuevo debe respetar el límite y el existente no empeorar. La cobertura desconocida o una referencia insuficiente impiden aprobar. Cambiar código, tests o condiciones obliga a renovar la evidencia afectada. `crap --scope src --test tests/test_subject.py` informa el estado actual sin comparación de tarea ni reparaciones.

`verify --control mutation --changes --test tests/test_subject.py` ejecuta la referencia funcional actual y los mutantes exigibles respecto del inicio guardado, sin requerir Full ni aprobación de DRY o C.R.A.P. También admite `--scope`. Conserva los mutantes preexistentes fuera del delta y los equivalentes comprobados; los inconclusos bloquean aunque el porcentaje alcance el umbral. Un inventario exigible vacío es `NO_APLICA`, sin inventar un score. `mutate --scope src --test tests/test_subject.py` analiza el estado actual completo del alcance, sin comparación incremental.

`verify` acepta la misma selección transitoria `--scope`/`--test` que `test`. Ambos admiten `--changes` para seleccionar código actual modificado respecto del inicio guardado, incluidos cambios preexistentes; puede combinarse con `--test`, pero no con `--scope`. Se informan también eliminaciones. Sin código medible actual (delta vacío, solo tests o eliminaciones) devuelve `delta_without_code`: elija explícitamente el código y los tests pertinentes. Sin `--test` conserva el comando del proyecto; no infiere automáticamente tests asociados.

```powershell
node .agentic-core/runtime-launcher.mjs agentic-quality verify --changes --test tests/test_subject.py
```
...
El motor interno utilizado por la verificación de tareas conserva su comparación con las fuentes originales de `active-task.json`: compara identidades de cuerpos, reconoce traslados y no oculta nuevas copias. Esa comparación no se aplica a la CLI de análisis actual; se solicita con `verify --control dry`. Si cambian los inputs o los límites, las resoluciones previas quedan obsoletas.
...
El gate local de calidad exige `prepare` antes de editar y `verify` antes de completar; los controles omitidos son `NO_SOLICITADO`. No se ejecuta el flujo del esquema 2 como sustituto. Esta selección
...
Esta sección describe la compatibilidad legacy del esquema 2; no es el contrato de tareas Python del esquema 3. Full se rechaza también en esta ruta.
...
La ruta histórica `QualitySession` rechaza `full` con un mensaje de deprecación. Las tareas Full existentes se conservan sin ejecución ni conversión; consulta [el archivo histórico](full-archive.md).
...
Las tablas siguientes corresponden a la interfaz legacy; en Python esquema 3 se usan `test`, `dry`, `crap` y `mutate` con la selección explicada arriba.
