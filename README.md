...
El adaptador conserva el cálculo de complejidad y C.R.A.P. del motor; delimita funciones y comportamiento de módulo sin contar dos veces sus cuerpos. Usa ramas atribuibles y, cuando no hay ramas, sentencias observadas para no convertir un cuerpo sin ejecutar en cobertura completa. La cobertura cero tiene un valor numérico; código no cargado, atribución ausente o ambigua, sintaxis no analizable y otros lenguajes incluidos en el alcance quedan `NO_VERIFICADO`, con resultados válidos de las demás partes. Las lambdas sin atribución separada también se informan como limitación. Las anotaciones de parámetros y retorno conservan una fila `annotation_coverage_unsupported`: la cobertura de cabecera no demuestra su evaluación independiente, aunque el cuerpo conserve una métrica válida. Esa limitación incluye anotaciones simples y stringizadas; la modalidad de evaluación usa la versión observada de pytest y queda desconocida si no se pudo observar. `NO_APLICA` exige ausencia comprobada de comportamiento ejecutable en el alcance. Un informe ajeno o divergente se conserva y produce un conflicto explícito.
