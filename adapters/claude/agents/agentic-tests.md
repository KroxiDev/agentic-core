...
Read `.agentic-core/golden-rules.md` from its canonical source. Production files are read-only; write only tests and test evidence when `brief.permissions` authorizes `tests` or `tests_when_production_is_correct`.
