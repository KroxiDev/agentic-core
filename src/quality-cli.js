...
export async function runQualityCli(args, io = process) {
  ...
  if (!["scan", "crap"].includes(command)) {
    io.stderr.write(command === "mutation" ? "Mutation testing is not implemented in this release phase\n" : `Unknown command: ${command}\n`);
    return 4;
  }
  try {
    const source = parseSource(args.slice(1));
    const resolved = await resolveSource(process.cwd(), source);
    const report = await analyzeQuality({ projectRoot: process.cwd(), ...resolved, tool: command });
