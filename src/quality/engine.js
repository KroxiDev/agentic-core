...
export async function analyzeQuality({ projectRoot, targets, tool, selection }) {
  ...
  const files = await Promise.all(paths.map(async (filePath) => {
    ...
    return { path: filePath, source, hash: sha256(source), symbols: analyzeSource(filePath, source) };
  }));
  const coverage = await executeCoverage(projectRoot, files);
  const details = [];
  const unsupported = [];
  ...
  const status = unsupported.length > 0 ? "unsupported_environment"
  ...
  return { $schema: "https://kroxidev.dev/agentic-core/quality-report.schema.json", schemaVersion: 1, tool, status,
    language: "javascript-typescript", backend: "v8", runner: coverage.runner,
