...
4. Después de fijar el alcance y antes de modificar producción o tests, ejecuta una sola vez `node .agentic-core/runtime-launcher.mjs agentic-quality prepare --mode <modo> --scope <ruta> [--scope <ruta>...]`. Conserva el ID devuelto y no inventes un baseline.
...
8. Si el Tester rechaza el resultado, agrupa los defectos y despacha una nueva instancia de Implementador con esos problemas, seguida de una nueva instancia de Tester. El contador compartido permite como máximo dos rondas adicionales por tarea, contando todos los roles. Al agotarlo, quedan causas pendientes sin aprobación ni cambio de modo. La documentación no se agrega sin petición expresa; al cerrar se puede recomendar un Documentador para decisión del usuario.
...
10. Ejecuta `node .agentic-core/runtime-launcher.mjs agentic-quality verify --session <id>` sobre el worktree final. Nunca declares completado un cambio ejecutable orquestado sin un recibo `QUALITY_OK` vigente. Operaciones destructivas, commit, push, publicación y cambios remotos requieren autorización explícita del usuario.
