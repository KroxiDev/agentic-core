...
const mutationJavaScriptProject = {
  manifest: {
    type: "module",
    scripts: { test: "node --test" },
  },
  files: {
    "src/subject.js": `export function choose(value) {
  if (value > 0) return true;
  return false;
}
`,
    "test/subject.test.js": `import assert from "node:assert/strict";
import test from "node:test";
...
const mutationPythonProject = {
  files: {
    "src/subject.py": `def choose(value):
    if value > 0:
        return True
    return False
...
def uncovered(value):
    return value + 0
`,
    "tests/test_subject.py": `import unittest
...
test("the Python catalogue emits every required valid mutation category", async (t) => {
  const root = await createTestProject(t, {
    ...mutationPythonProject,
    files: {
      ...mutationPythonProject.files,
      "src/subject.py": `def catalogue(a, b):
    enabled = True
    missing = None
    label = "value"
    zero = 0
...
test("Python mutate shares mutation states and leaves the working tree untouched", async (t) => {
  const root = await createTestProject(t, mutationPythonProject);
...
test("Python mutation distinguishes baseline and mutant timeouts", async (t) => {
  const baselineRoot = await createTestProject(t, {
    ...mutationPythonProject,
    files: {
      ...mutationPythonProject.files,
      "tests/test_subject.py": "while True: pass\n",
    },
  });
  ...
  const mutantRoot = await createTestProject(t, {
    ...mutationPythonProject,
    files: {
      ...mutationPythonProject.files,
      "src/subject.py": `def countdown(value):
    while value < 0:
        value -= 1
    return value
`,
...
test("Python mutation executes pytest when the project declares it", async (t) => {
  const root = await createTestProject(t, {
    ...
    files: {
      ...mutationPythonProject.files,
      "tests/test_subject.py": `import pytest
from src.subject import choose
...
test("mutate reports survived and uncovered mutants and never changes the working tree", async (t) => {
  const root = await createTestProject(t, mutationJavaScriptProject);
  await mkdir(path.join(root, "src", "generated"));
...
test("a failed baseline invalidates mutation analysis with its stable exit code", async (t) => {
  const root = await createTestProject(t, {
    ...mutationJavaScriptProject,
    files: {
      ...mutationJavaScriptProject.files,
      "test/subject.test.js": `import assert from "node:assert/strict";
import test from "node:test";
...
test("an exhausted baseline also invalidates the complete analysis", async (t) => {
  const root = await createTestProject(t, {
    ...
    files: {
      ...mutationJavaScriptProject.files,
      "test/subject.test.js": "while (true) {}\n",
    },
  });
  const result = await run(["mutate", "--target", "src/subject.js"], root, {
    env: { ...process.env, NODE_ENV: "test", AGENTIC_CORE_TEST_BASELINE_TIMEOUT_MS: "100" },
...
test("a mutant timeout is distinguished as killedByTimeout", async (t) => {
  const root = await createTestProject(t, {
    ...
    files: {
      ...mutationJavaScriptProject.files,
      "src/subject.js": `export function countdown(value) {
