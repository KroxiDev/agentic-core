...
test("Windows distributed package completes the independent consumer lifecycle", {
  ...
  const maintenance = async (args) => {
    const result = await execute(process.execPath, [installer, ...args, root], {
  ...
  try {
    ...
    parse(await run(["verify"]));
    assert.ok(Number(await count()) > Number(calls));
    await writeFile(source, original);
    ...
    await assert.rejects(lstat(path.join(root, ".agentic-core/quality/verification.json")), { code: "ENOENT" });
